...
| `state.delete` | namespace=`profiles`\|`vault`, key, entry?(仅 profiles) | ok + deleted 数 | 🔒 `state:delete:<ns>` | P4 |

**vault 特例**：`vault://` 对 Agent 永远**只写不读**。Agent 在 `act.type` 中传 `vault_ref`，
由内核在引擎侧替换真实凭据——LLM 上下文永不出现明文密钥。值长度不限，出口脱敏
分级：≥ 6 字节的值做全链路**子串**遮蔽；更短的值只做**整值精确匹配**遮蔽（盲目
子串替换会把含同子串的无关数据——如凭据名——误伤成 `[REDACTED]`）；vault 写入的
`value` 在 journal 里另有**结构化遮蔽**兜底（无论长短）。凭据可经 `state.delete`
（namespace=`vault`，key=凭据名）删除；删除**不回收**出口脱敏登记（历史 journal
与引擎侧可能仍有痕迹，脱敏终身有效）。

**profiles 特例**（ADR-0011）：`state.import` 导入的登录状态（cookie/storage）同样是
凭据。`state.list`（namespace=`profiles`）只回 profile 名；`state.read`（key=profile 名）
只回**元数据摘要**——entry 键名、类别、cookie 的 domain/path/secure/httpOnly、值字节数，
值本身绝不进入任何返回。删除走 `state.delete`（带 `entry` 只删单条，缺省整删），
只作用于 profile 存储，不触碰运行中的进程。
...
| P4 | 导入状态与凭据的可观察与可删除（见 [ADR-0011](adr/0011-profile-state-inspect-delete.md)）：新方法 `state.delete`（namespace=`profiles` 整删/按 `entry` 单删；namespace=`vault` 按凭据名删除，脱敏登记不回收）；`state.list`/`state.read` 新增 `profiles` 命名空间（list 只回名字，read 只回元数据摘要，值绝不回流）；敏感作用域 +`state:delete`；`ABI_VERSION` 0.3.0 → 0.4.0 | 方法表 48 → 49；sensitive scope +1（`state:delete`）；wire 封装不变 |
