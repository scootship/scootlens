...
`console.log`、`dialog.open`、`download.*`、`cap.request`

### cap — 能力

| 调用 | 参数 | 返回 | 所需能力 | 阶段 |
|---|---|---|---|---|
| `cap.request` | scopes[], reason | 审批结果/待审 | —（任何主体可申请） | P2 |
| `cap.list` | token? | 已授予作用域 | `cap:read` | P2 |
| `cap.grant` / `cap.revoke` | subject, scopes[] | ok | 🔒 `cap:admin`（Console/管理员） | P2 |

### wf — 工作流

| 调用 | 参数 | 返回 | 所需能力 | 阶段 |
|---|---|---|---|---|
| `wf.create` / `wf.list` / `wf.run` / `wf.cancel` | spec / id | id/状态 | `wf:manage` | P3 |
...
| `obs.replay.export` | pid, range | 回放包 | 🔒 `obs:replay` | P4 |
