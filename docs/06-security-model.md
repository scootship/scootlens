...
`js:exec`、`state:read/write/export/import/delete`、`act:upload`、`act:takeover`、`net:rules`、
...
| T2 | 凭据外泄至 LLM/日志 | act.type、journal | vault 只写不读 + 注入瞬间解引用；journal/trace 全链路脱敏，分级遮蔽：≥6 字节的值子串替换，更短的值整值精确匹配（盲目子串替换会把含同子串的无关数据误伤成 `[REDACTED]`）；vault 写入的 value 在 journal 结构化遮蔽兜底 |
